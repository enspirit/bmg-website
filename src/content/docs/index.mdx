...
<CardGrid>
        ...
        <LinkCard
          title="Want to use Bmg in Ruby?"
          description="Get started in no time with our tutorials and operations reference."
          href="/usage/getting-started/"
        >
        </LinkCard>
        <LinkCard
          title="Want to use Bmg.js in TypeScript?"
          description="Bring relational algebra to your JavaScript/TypeScript projects."
          href="/usage/getting-started-ts/"
        >
        </LinkCard>
</CardGrid>
...
// for each product in the cheese category
purchases
  .exclude({ product_type: 'sample' })
  .join(products, { product_id: 'id' })
  .restrict({ category: 'cheese' })
  .project(['product_id', 'quantity'])
