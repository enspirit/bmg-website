...
**Based on an underlying set**: The sets in our previous examples were integers, food, and strings. Relational algebra is defined over the **set of relations**. We will discuss relations in the next section, but for now you can think of a relation as a database table (even if important differences exist). In other words the *values* we operate on are *relations* aka *tables*.
