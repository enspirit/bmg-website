...
The *heading* of the table defines the names of the attributes and their domains (or types). Each row represents a tuple and each column lists one attribute of the tuples.
...
The first problem is that the attributes in both relations have the same names (namely **Name** and **YearsKnown**). Since attribute names must be unique, we start by usng the **Rename** operator, denoted **ρ**. **ρ(X/Y)** means that we rename the attribute **Y** to **X**. 
...
In Bmg, this whole transformation would look like this:

```ruby
my_friends2 = my_friends.rename(:name => :name1, :years_known => :years_known2)
your_friends2 = your_friends.rename(:name => :name2, :years_known => :years_known2)
my_friends2.cross_product(your_friends2)
           .restrict(Predicate.eq(:name1, :name2))
           .project([:years_known1, :years_known2)
```
