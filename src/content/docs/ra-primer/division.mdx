...
Let's bring back the example of relations describe the wearing of garments from the section on relations. We start with the following two relations:
...
We form the cross product **R = P x Q**:
...
1. Both the divisor and the quotient can have one or more attributes in their header
1. The headers of the divisor and quotient must each be a subset of the header of the dividend, and they may not overlap
1. Thus, the header of the quotient consists of exactly those attributes of the dividend that are not in the divisor
1. The tuples in the quotient are simply what you get if you project its attributes from the divisor

In this example, we could get the quotient of **R3 ÷ R1** by noticing that the quotient's header has a single attribute (`color`), which we get by removing the attributes of **R1** from the attribute list of **R3**. Using Bmg, we can get the quotient like this: `r.project([:color])` and since tuples in a relation are unique, with no duplicates, this will yield exactly **R2** as shown above.
...
(We have omitted the `wearing` attribute, but the intended meaning is still that *a person with a particular name is wearing a particular color*.)
...
We know the header of **Q** is `(name)`, so we can start by simply projecting this from **R** to form an intermediate relation, **Q0**:
...
We can of course also as the opposite question:
...
> *Which recipies contain all specified ingredients?*

> *Which candidates have all of the required skills?*

> *Are there any team members that have worked on all of these sub-projects?*
...
https://www.red-gate.com/simple-talk/databases/sql-server/learn/high-performance-relational-division-in-sql-server/
