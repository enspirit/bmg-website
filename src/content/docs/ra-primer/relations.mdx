...
This is what a relation means in a relational database. In the example, those combinations of `person`, `garment`, and `color` for which the relation **wearing** holds.
...
:::note
From a purist point of view, it is more correct to name database tables eg `user` rather than `users`, because a table is **a relation** rather than a list of things. But then again, an extensional definition of the `user` relation is precisely... a set of users.
:::
