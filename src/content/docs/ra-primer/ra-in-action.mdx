...
:::note
Although we use Bmg in this section and the two remaining ones, we are only interested in presenting the fundamentals of relational algebra. The code shown here does not represent best practices for using Bmg!

Furthermore, we only consider the basic form of relational algebra here, without extensions like aggregations or outer joins. Those are described in [Advanced features](TODO), [SQL Interface](TODO), and in the [Operations Reference](http://localhost:4321/reference/overview/).
:::
...
The new header contains all the attributes from both relations, except that **name** from `films` and **film_name** from `role` have been merged. That makes sense, because what we have done is precisely to pick out those tuples where one equals the other. To include both would be redundant.
