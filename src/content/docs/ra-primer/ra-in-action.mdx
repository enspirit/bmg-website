...
## Join = Rename + Cross product + Project
...
The idea is to perform a **Cross product** to get **all** the combinations of tuples, followed by a **Restrict** to pick out the combinations that have matching attributes. However, since **Cross product** expects unique attributes (whereas **Natural join** expects at least one attribute to be shared), we first have to rename attributes. And as a last step, we remove the extra attributes created by the rename. Let's go through this step by step:
