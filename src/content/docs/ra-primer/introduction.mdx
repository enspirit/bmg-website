...
**Bmg** is an implementation of relational algebra in Ruby. It can be used to query and transform in-memory datasets, which is very convenient in a range of situations, but not a replacement for a proper database system. Therefore, Bmg can also be used in conjunction an external SQL database, in which case Bmg functions as a powerful and very flexible interface to the underlying database.
