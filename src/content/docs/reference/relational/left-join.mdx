...
```typescript
left.left_join(right, ['a', 'b', ...])
left.left_join(right, ['a', 'b', ...], defaultRightTuple)
left.left_join(right, { a: 'x' }, defaultRightTuple)
```
  </TabItem>
</Tabs>
...
- Attributes from the right relation are filled with values from the `default_right_tuple` (or `null`/`nil` if not specified)
...
```
  ...
  <TabItem label="TypeScript">
```typescript
suppliers.left_join(cities, ['city']).toArray()
...
```typescript
// Using a relation where some parts have cities not in the cities table
parts.left_join(cities, ['city'], { country: "Unknown" }).toArray()
